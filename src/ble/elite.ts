...
BleInterface.register('BleCyclingPowerDevice','cp', BleEliteDevice,BleEliteDevice.services)

export class EliteAdapter extends DeviceAdapter {
    ...
    constructor( device: BleDeviceClass, protocol: BleProtocol) {
        super(protocol);
        this.device = device as BleEliteDevice;
    ...
    isSame(device:DeviceAdapter):boolean {
        if (!(device instanceof EliteAdapter))
            return false;
        const adapter = device as EliteAdapter;
