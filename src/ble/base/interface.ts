...
type TStateChangeHandler = (state:BleInterfaceState)=>void
/**
...
export class BleInterface   extends EventEmitter implements IBleInterface<BlePeripheralAnnouncement> { 
    ...
    */
     ...
     */
    async connectInternal(reconnect?:boolean): Promise<boolean> {

        if (this.isConnected())
            return true;

        //this.logEvent( {message: 'Trigger BLE autoconnect', state:this.getBinding().state})
    ...
    */
    ...
    protected waitForBleConnected():Promise<boolean> {
        return new Promise<boolean>( (done)=>{
            ...
            this.on('ble-state-change', (state:BleInterfaceState) => {
                try {
    ...
    protected async onBleStateChange(state:BleInterfaceState) {
