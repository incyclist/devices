...
describe( 'ANT SPD adapter', ()=>{
    describe('constructor',()=>{
        test('typical settings, empty props',()=>{
            ...
            const adapter = new AntSpdAdapter(settings,{})

            // simple getters
            expect(adapter.getID()).toBe('2606')                        
            expect(adapter.getName()).toBe('XXXX')
            expect(adapter.getCapabilities()).toEqual([ IncyclistCapability.Speed])
        ...
        test('minimal settings',()=>{
            ...
            // simple getters
            expect(adapter.getID()).toBe('2606')                        
            expect(adapter.getName()).toBe('Ant+SPD 2606')
            expect(adapter.getCapabilities()).toEqual([ IncyclistCapability.Speed])
        ...
        test('incorrect profile',()=>{
            ...
            let adapter,error;
            try {
                adapter = new AntSpdAdapter(settings,{})
            }
            catch(err) {
                error = err;
            }
    ...
    describe('getUniqueName',()=>{
        let adapter;
        beforeEach( ()=>{            
            adapter = new AntSpdAdapter({deviceID: '2606',profile: 'SPD',interface: 'ant'})

        })

        test('no data (yet)',()=>{
            expect(adapter.getUniqueName()).toBe('Ant+SPD 2606')
        })
        ...
        test('has received ManId',()=>{
            ...
            expect(adapter.getUniqueName()).toBe('Polar SPD 2606')
        })
