...
class Daum8i {
    constructor(props) {
        const opts = props || {};
        this.logger = props.logger || new gd_eventlog_1.EventLogger('DaumPremium');
    ...
    onData(data) {
        ...
        for (let i = 0; i < incoming.length; i++) {
            ...
            if (c === 0x06) {
                ...
                const remaining = getRemaining();
                this.logger.logEvent({ message: "sendCommand:received:", port: portName, cmd: `${cmd} [${utils_1.hexstr(cmd)}]`, remaining: utils_1.hexstr(remaining) });
                ...
                this.state.sending.timeoutCheckIv = setInterval(() => {
                    this.checkForTimeout((err) => {
                        ...
                        setTimeout(() => {
                            ...
                            this.logger.logEvent({ message: "sendCommand:retry:", port: this.portName, cmd: command, hex: utils_1.hexstr(message) });
                            port.write(message);
