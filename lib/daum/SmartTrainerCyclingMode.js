...
const config = {
    ...
    properties: [
        ...
        { key: 'minPower', name: 'Minimum Power', description: 'minimum power in desclines', type: CyclingMode_1.CyclingModeProperyType.Integer, default: 50 },
    ]
};
class SmartTrainerCyclingMode {
    constructor(adapter, props) {
        this.prevUpdateTS = 0;
        this.hasBikeUpdate = false;
        this.settings = {};
        this.setAdapter(adapter);
        this.logger = new gd_eventlog_1.EventLogger('SmartTrainerMode');
        this.settings = props || {};
    }
    setAdapter(adapter) {
    ...
    calculateTargetPower(request, updateMode = true) {
        ...
        const prevData = this.prevData || {};
        if (prevData.speed !== undefined && prevData.slope !== undefined) {
            const v = prevData.speed / 3.6;
            let power = calculations_1.default.calculatePower(m, v, prevData.slope);
            if (power < minPower)
                power = minPower;
