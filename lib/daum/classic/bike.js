...
class Daum8008 {
    ...
    getUserWeight() {
        if (this.settings === undefined || this.settings === null || this.settings.user === undefined || this.settings.user === null)
            return utils_1.getWeight(undefined);
        else
            return utils_1.getWeight(this.settings.user.weight) - 10;
    }
    getBikeWeight() {
    ...
    checkCockpit(bikeNo = 0) {
        return new Promise((resolve, reject) => {
            this.sendDaum8008Command(`checkCockpit(${bikeNo})`, [0x10, bikeNo], 3, (data) => resolve({ bike: data[1], version: data[2] }), (status, err) => reject(utils_1.buildError(status, err)));
        });
    }
    getAddress() {
        return new Promise((resolve, reject) => {
    ...
    setPerson(user = {}, bikeNo = 0) {
        ...
        const weight = utils_1.getWeight(user.weight) + 10;
        ...
        cmd.push(0);
        return new Promise((resolve, reject) => {
            this.sendDaum8008Command(`setPerson(${bikeNo},${age},${gender},${length},${weight})`, cmd, 16, (data) => resolve({ bike: data[1], age, gender, length, weight }), (status, err) => reject(utils_1.buildError(status, err)));
        });
    }
    runData(bikeNo = 0) {
        return new Promise((resolve, reject) => {
            this.sendDaum8008Command(`runData(${bikeNo})`, [0x40, bikeNo], 19, (data) => resolve(utils_1.parseRunData(data)), (status, err) => reject(utils_1.buildError(status, err)));
        });
    }
